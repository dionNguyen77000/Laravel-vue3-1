                        ...
                        <tr v-for="record in filteredRecords" :key="record"  class="border-b border-gray-200 bg-gray-50 hover:bg-gray-100">
                            ...
                            <td class="py-2 text-left"  v-if="!hideColumns.includes(column)">
                                ...
                                <template v-if="editing.id === record.id && isUpdatable(column)">
                                    ...
                                    <template v-else>
                                        <input type="text"  
                                        class="w-3/4 rounded-r rounded-l sm:rounded-l-none border border-gray-400 pl-1 pr-1 py-1 bg-white text-sm text-gray-700 focus:bg-white"
                                ...
                                <template v-else>
                                    <template v-if="column=='parent_id'">
                                        ...
                                        <span class="font-medium" >{{response.categoryParentOptions[columnValue]}}</span>
                                    </div>
                                    </template>
                                    <template v-else>
                                    <div class="flex items-center">
...
export default {
 ...
 computed: {
   ...
   methods: {
            ...
            getRecords(){
                return axios.get(`/api/datatable/categories?${this.getQueryParameters()}`).then((response)=> {
            ...
            update () {
                axios.patch(`/api/datatable/categories/${this.editing.id}`, this.editing.form).then((response) => {
            ...
            destroy(record){
                ...
                axios.delete(`/api/datatable/categories/${record}`).then(()=>{
                    this.selected= [],
