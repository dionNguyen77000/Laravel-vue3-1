      ...
      <button @click="closeModal" type="button" 
        ...
        <div class="min-w-screen min-h-screen bg-gray-100 flex justify-center rounded-lg shadow-md">
            ...
            <div class="flex justify-between pt-4">
                <div class="text-2xl font-semibold uppercase"> Compare Order and Invoice</div>
                <!-- <div>
                    <a href="#" 
                    ...
                    <tbody class="text-gray-600 text-sm font-light">
                        ...
                        <tr v-for="record in filteredRecords" :key="record"  class="border-b border-gray-200 bg-gray-50 hover:bg-gray-100">
                            ...
                            <td class="py-2 text-left"  v-if="!hideColumns.includes(column)">
                                ...
                                <template v-else>
                                     ...
                                     <template v-else-if="column=='i_unit_price'">
                                        <div :class="{ 'text-center': textCenterColumns.includes(column), 
                                        'bg-pink-500': record.o_unit_price!=record.i_unit_price }" >   
                                            <span class="font-medium" >{{columnValue}}</span>
                                        </div>
                                    </template>
                                     <template v-else-if="column=='i_line_price'">
                                        <div :class="{ 'text-center': textCenterColumns.includes(column), 
                                        'bg-pink-500': record.o_line_price!=record.i_line_price }" >   
                                            <span class="font-medium" >{{columnValue}}</span>
                                        </div>
                                    </template>
                        ...
                        <tr class="border-b border-gray-200 bg-gray-200 hover:bg-gray-10">
                            <td colspan="100%" class="p-2 text-center text-red-900 text-2xl">
                                Estimated Order: <b class="font-bold">${{estimated_order_total_price}}</b>
                                 Estimated Invoice: <b class="font-bold">${{estimated_invoice_total_price}}</b>
                                Invoice Total: <b class="font-bold">${{invoice_total_price}}</b>
                            </td>
                            <!-- <td colspan="50%" class="p-2 text-center text-red-900 text-2xl">
                                Invoice: <b class="font-bold">${{order_estimated_price}}</b>
                            </td> -->
                        </tr>
                    </tbody>
                </table>
...
export default {
    props: ['orders_to_supplierId','invoice_total_price','order_estimated_price'],
   data() {
            return {
                response: {
                    table: '',
 ...
 computed: {
    ...
    ...mapState(['sideBarOpen']),
        ...
        estimated_order_total_price(){
            return this.response.records.reduce((sum, record) => {
                return sum += record.o_line_price;
            }, 0);
        },
