  ...
  <div id="delivery_setting" class="p-6"> 
        <div class="min-w-screen min-h-screen bg-gray-100 flex justify-center rounded-lg shadow-md">
            ...
            <div class="flex justify-between pt-4">
                <div class="text-2xl font-semibold uppercase"> Table Deilvery Setting</div>
                <div>
                    <a href="#" 
...
export default {
  middleware: [
   ...
   data() {
            return {
                ...
                textCenterColumns:['suburb','zone','duration_one_direction','delivery_fee','fuel_cost'],
                dollarsSymbolColumns:['fuel_cost','delivery_fee'],
 ...
 computed: {
   ...
   methods: {
            ...
            getRecords(){
                return axios.get(`/api/datatable/delivery_settings?${this.getQueryParameters()}`).then((response)=> {
            ...
            update () {
                this.isLoading = true
                axios.patch(`/api/datatable/delivery_settings/${this.editing.id}`, this.editing.form).then((response) => {
            ...
            store () {    
                this.isLoading = true
                axios.post(`/api/datatable/delivery_settings`, this.creating.form).then((response) => {
                    this.isLoading = false
                    if(this.image) {
                        ...
                        //save image 
                        axios.post(`/api/datatable/delivery_settings/saveImage/${response.data.id}`, data).then(()=>{
                        this.getRecords().then(() => {                               
                            this.image =null;
            ...
            exportPDF(){
                axios.get(`/api/datatable/delivery_settings/createPDF`,
            ...
            destroy(record){
                ...
                axios.delete(`/api/datatable/delivery_settings/${record}`).then(()=>{
                    this.isLoading = false
                    this.selected= [],
            ...
            saveImage(productId) {
                ...
                if(this.image){
                    ...
                    data.append('image', this.image)
                this.isLoading = true
                axios.post(`/api/datatable/delivery_settings/saveImage/${productId}`, data).then((response)=>{
                    this.isLoading = false
